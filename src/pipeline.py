...
def pipeline_single(input_fastq_filename: str, db: str, wsl: bool):
    ...
    run_consensus(base_name, input_fastq_path, consensus_method, wsl=wsl)
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python3 pipeline.py <name_of_the_input_fastq_file> <wsl (True/False, Optional)> <db (Optional)>")
        sys.exit(1)
    ...
    db = None
    if len(sys.argv) >= 3:
        wsl = True if sys.argv[2].lower() in ["true", "t", "yes", "y"] else False
    if len(sys.argv) >= 4:
        db = sys.argv[3]
